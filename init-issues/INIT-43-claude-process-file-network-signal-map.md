...
- [x] Empirical run artifacts linked. Claude Cowork (same process/file/network discipline): LAB-RUN-014 protocol and results, LAB-RUN-015 gap scenario results (2026-03-16; protocol-expected). Lab run files were previously in `lab-runs/` (removed); calibration fixtures remain in `collector/tests/fixtures/lab_runs/LAB-RUN-014.json` and `LAB-RUN-015.json`.
