...
**User management (March 9, 2026):** Added full user CRUD system. Backend: new `/users` router with list, create, get, update, and deactivate endpoints (owner/admin gated). Four-role model (owner, admin, analyst, viewer) with guard rails (owner is immutable, cannot deactivate self, cannot assign owner role). Migration 0005 splits `full_name` into `first_name`/`last_name`, adds `auth_provider` and `password_reset_required` columns for future SSO/email provisioning. Frontend: Admin page with users table, search, pagination, add-user modal, edit modal, deactivate toggle, and color-coded role badges. TopBar and LoginPage updated for split name fields.
