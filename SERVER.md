...
Agents can connect via either transport (or both over time with **auto**):
- **Auto** (agent default): tries TCP first, falls back to HTTP if the gateway is blocked; may recover to TCP when the gateway is reachable again.
- **HTTP**: REST calls to `POST /api/events` and `POST /api/endpoints/heartbeat` on port 8000.
- **TCP binary protocol**: Persistent msgpack-framed connection to the DetecGateway on port 8001. Lower overhead, supports server-push (policy updates, commands), and automatic reconnection. Use `--protocol tcp` for TCP only, or auto for resilient installs. Set `GATEWAY_ENABLED=true` on the server.
...
Server-side capabilities use the same four-label taxonomy as [docs/product-status.md](docs/product-status.md). Labels here describe operator-relevant server features.

| Capability | Maturity | Notes |
|------------|----------|-------|
| HTTP event ingest (`POST /api/events`) | **GA** — Live-validated | Core ingest path; used by all agents |
| Binary TCP gateway (port 8001) | **GA** — Live-validated | Persistent agent connections, server-push (policy, commands). Requires `GATEWAY_ENABLED=true`. |
| SQLite database | **GA** — Live-validated | Zero-config; suitable for ≤10 endpoints |
| PostgreSQL database | **GA** — Live-validated | Recommended for production and multi-tenant deployments |
| Dashboard (management UI) | **GA** — Live-validated | Pre-built React SPA served at root URL |
| Docker Compose deployment | **GA** — Live-validated | Includes PostgreSQL; see Option A |
| Windows Server installer | **GA** — Live-validated | GUI installer (`DetecServerSetup.exe`); see Option B |
| Alembic schema migrations | **GA** — Live-validated | Auto-run on startup; manual control via `RUN_MIGRATIONS=false` |
| TLS termination | **Partial** — Protocol-expected | Supported via reverse proxy or `TLS_CERT_FILE`/`TLS_KEY_FILE`; not enabled by default |
| CrowdStrike EDR enrichment | **Experimental** — Protocol-expected | Provider scaffolding present; not full-parity with native tactics. Explicitly non-GA. |
| Native ESF / ETW / eBPF telemetry | **Experimental / Roadmap** | Provider scaffolding and tests exist; not active in production baseline |

Capability maturity label definitions (verbatim from [docs/product-status.md](docs/product-status.md)):
- **Live-validated:** verified in completed lab runs and/or active runtime tests.
- **Protocol-expected:** evidence comes from fixtures, protocol runs, or synthetic harnesses; not equivalent to full live validation.
- **Experimental:** implemented partially or not yet supported for broad production use.
...
This produces `installers/windows/dist/DetecServerSetup-0.1.0.exe`. The installer uses a full dark theme (Slate 900) and walks through: Welcome, License, Pre-flight checks, Server configuration (port, database), Admin account, Summary, Installation, and Finish. See [installers/windows/README.md](installers/windows/README.md) for full details.
...
Or build manually (see [installers/windows/README.md](installers/windows/README.md)).

### 2. First-run setup

```powershell
cd installers\windows\dist\detec-server
...
The API includes several hardening measures for production use. For a concise checklist (headers, error handling, secrets, playbook/audit), see [docs/hardening-checklist.md](docs/hardening-checklist.md). For a security assessment and findings, see [docs/SECURITY-TECHNICAL-REPORT.md](docs/SECURITY-TECHNICAL-REPORT.md).
...
| `GATEWAY_TLS_CERT` | _(none)_ | Path to a PEM certificate file for TLS. In `production`/`staging`, this is required when the gateway is enabled. |
...
**Docker:** Installers are **baked into the API image** from `docker/default-agent-packages/` when you run `docker compose build`. Copy Windows and Linux artifacts there, then rebuild the **api** image. Optional: uncomment the `./dist/packages` volume in `docker-compose.yml` to override without rebuilding (dev only).

**Bare metal / PyInstaller server:** Place pre-built agent packages in `dist/packages/` relative to the API working directory (or the server exe directory for PyInstaller builds):
...
macOS endpoints use `pip install -e .` and `detec-agent` from source; there is no server-side macOS package download.
...
## Evasion and enforcement controls

For post-deploy verification and rollback of evasion-related changes (tamper floor, enforcement safety gating, evasion CI suite), see [docs/evasion-controls-rollback.md](docs/evasion-controls-rollback.md). That runbook covers API/collector compatibility checks, enforcement safety (e.g. Linux UID fallback gating), evasion regression tests, and rollback steps.
