...
We discover and control autonomous AI tools by what they do, not what they're called. We score every detection with explainable confidence and enforce policy on a ladder you control: visibility, warning, approval required, or block. We also reconstruct agent sessions — showing the concrete sequence of actions an agent took (LLM request → shell exec → file write → git commit) — so analysts can review what happened, not just that something was detected. We publish our known limits and test for evasion, because transparency about what we can't do builds more trust than perfection claims. Detection confidence varies by tool and environment; high-risk tools are often reported at Medium confidence without EDR or kernel telemetry.

---

## What we believe

**Evidence over assertion.** Every claim we make is traceable to signals, lab results, or documented methodology. If we can't show the evidence, we don't make the claim.

**Proportional governance.** Governance is a spectrum, not a switch. Low-risk tools get visibility. High-risk tools get gates. Your policy defines the thresholds.

**Earned trust.** We publish known limits, evasion test results, and honest gaps. Trust is earned, not assumed.

**Operator-first design.** The person using Detec every day is the most important user. The product is built for their workflow, not for a demo.

---

## The name

Detec comes from "detect." The missing letter is intentional: detection is where governance starts, and the name reflects that. Pronounced dee-TEK.

---

## Next step

Start a pilot, contact sales, or see how it works. *(Replace with your primary CTA and link.)*
