...
export default function useEndpoints() {
  ...
  const load = useCallback(async (filterOverrides) => {
    ...
    // Cookie sessions use httpOnly cookies (no localStorage token). API key is optional.
    // Always attempt load; apiFetch uses credentials: 'include' so cookies are sent.
