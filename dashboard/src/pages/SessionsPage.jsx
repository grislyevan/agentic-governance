...
const VERDICT_COLORS = {
  ...
  interesting: 'bg-detec-ui-accent/10 text-detec-ui-accent',
  risky: 'bg-amber-500/15 text-amber-400',
  high_risk: 'bg-detec-enforce-block/15 text-detec-enforce-block',
};

function VerdictBadge({ verdict }) {
  if (!verdict) return <span className="text-detec-ui-muted text-xs">-</span>;
  const colors = VERDICT_COLORS[verdict] || 'bg-detec-slate-200 text-detec-ui-muted';
...
function ActionsSummary({ report }) {
  ...
  if (ts && typeof ts === 'object' && Object.keys(ts).length > 0) {
    const parts = Object.entries(ts)
      ...
      .map(([k, n]) => `${k}(${n})`)
      .slice(0, 5);
    return <span className="text-xs text-detec-ui-text font-mono">{parts.join(', ') || '-'}</span>;
  }
  ...
  return <span className="text-detec-ui-muted text-xs">-</span>;
}
          ...
          <table className="w-full text-left min-w-[640px]" aria-label="Agent sessions">
            <thead>
              <tr className="bg-detec-ui-surface/80 border-b border-detec-ui-border/50">
                <th scope="col" className="px-3 sm:px-4 py-3 text-xs font-medium text-detec-ui-muted uppercase tracking-wider">Time</th>
                <th scope="col" className="px-3 sm:px-4 py-3 text-xs font-medium text-detec-ui-muted uppercase tracking-wider">Endpoint</th>
                <th scope="col" className="px-3 sm:px-4 py-3 text-xs font-medium text-detec-ui-muted uppercase tracking-wider">Tool</th>
                <th scope="col" className="px-3 sm:px-4 py-3 text-xs font-medium text-detec-ui-muted uppercase tracking-wider">Verdict</th>
                <th scope="col" className="px-3 sm:px-4 py-3 text-xs font-medium text-detec-ui-muted uppercase tracking-wider">Confidence</th>
                <th scope="col" className="px-3 sm:px-4 py-3 text-xs font-medium text-detec-ui-muted uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody>
              {reports.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center">
                    <p className="text-detec-ui-muted text-sm font-medium">No session reports yet</p>
                    <p className="text-detec-ui-muted text-xs mt-1">Sessions appear when detection events are aggregated by endpoint and tool.</p>
                  </td>
                </tr>
              ) : (
                reports.map((report) => {
                  ...
                  >
                    <td className="px-3 sm:px-4 py-3 text-sm text-detec-ui-text whitespace-nowrap">
                      {report.started_at ? new Date(report.started_at).toLocaleString() : '-'}
                    </td>
                    <td className="px-3 sm:px-4 py-3 text-sm text-detec-ui-text font-mono truncate max-w-[120px]" title={report.endpoint_id}>
                      {report.endpoint_id || '-'}
                    </td>
                    <td className="px-3 sm:px-4 py-3 text-sm text-detec-ui-text">{report.tool || '-'}</td>
