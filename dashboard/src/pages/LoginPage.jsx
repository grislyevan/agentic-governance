      ...
      <div className="w-full max-w-sm space-y-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="rounded-detec-md border border-detec-ui-border/50 bg-detec-surface/80 p-6 space-y-4">
              ...
              <>
                <div className="grid grid-cols-2 gap-3">
                  <label className="block space-y-1.5">
                    <span className="text-xs font-medium text-detec-ink-secondary uppercase tracking-wider">First name</span>
                    <input
                      type="text"
                      value={firstName}
                  ...
                  <label className="block space-y-1.5">
                    <span className="text-xs font-medium text-detec-ink-secondary uppercase tracking-wider">Last name</span>
                    <input
                      type="text"
                      value={lastName}
                ...
                <label className="block space-y-1.5">
                  <span className="text-xs font-medium text-detec-ink-secondary uppercase tracking-wider">Organization</span>
                  <input
                    type="text"
                    value={tenantName}
            ...
            <label className="block space-y-1.5">
              <span className="text-xs font-medium text-detec-ink-secondary uppercase tracking-wider">Username</span>
              <input
                type="text"
                value={email}
            ...
            <label className="block space-y-1.5">
              <span className="text-xs font-medium text-detec-ink-secondary uppercase tracking-wider">Password</span>
              <input
                type="password"
                value={password}
            ...
            {mode === 'login' && (
              <div className="text-center">
                <button
                  ...
                  className="text-xs text-detec-ink-secondary hover:text-detec-brand transition-colors"
                >
                  Forgot password?
                </button>
              </div>
            ...
            {ssoAvailable && mode === 'login' && (
              <>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-detec-ink-secondary uppercase tracking-wider">or</span>
                  <div className="flex-1 border-t border-detec-ui-border" />
                </div>
                <a
                  href="/api/auth/sso/login"
                  className="block w-full px-4 py-2.5 border border-detec-ui-border hover:border-detec-brand/50 text-detec-ink-primary text-sm font-medium rounded-detec-md transition-colors text-center"
                >
                  Sign in with SSO
                </a>
              </>
          ...
          <p className="text-center text-sm text-detec-ink-secondary">
            {mode === 'login' ? (
              <>
                No account?{' '}
                <button type="button" onClick={() => { setMode('register'); setError(''); }} className="text-detec-brand hover:text-detec-brand transition-colors">
                  Create one
                </button>
              </>
            ) : (
              <>
                ...
                <button type="button" onClick={() => { setMode('login'); setError(''); }} className="text-detec-brand hover:text-detec-brand transition-colors">
                  Sign in
                </button>
              </>
            )}
