...
| Claude Cowork | **Standard install + launch + session analysis + teardown (CW-POS-01)**, MCP connector agentic task, scheduled task exercise (CW-POS-02), DXT chrome-control (CW-POS-03), skill-creator. Protocol for gap scenarios: LAB-RUN-015. | Non-standard install, VPN-routed API, DXT extension modification | `IN PROGRESS` (1/3 positive; 015 protocol ready for CW-POS-02/03/skill-creator) |
...
| LAB-RUN-015 | (protocol) | Claude Cowork | CW-POS-02 (scheduled task), CW-POS-03 (DXT chrome-control), skill-creator self-modification | **Pending** | Protocol only. Defines phases 3D, 3E, 3F with capture blocks and INIT-43 required outputs. Run when gap scenarios are executed. Protocol: `lab-runs/LAB-RUN-015-claude-cowork-scheduled-dxt-skill.md`. |
