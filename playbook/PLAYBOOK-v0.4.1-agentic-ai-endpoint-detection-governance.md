...
**Version:** 0.4.1 — Rule ID Catalog + Enforcement Pipeline + Weight Alignment  
...
|---|---|
| Detection profiles: 12 tools (INIT-13–22 + OpenClaw + Claude Cowork) | Test matrix definition (INIT-28) |
...
| Signal map deep-dive (INIT-43, complete) | Detection content update process (INIT-42) |
...
**Behavioral layer caveat:** The behavioral layer is effective for **long-running interactive sessions** where process and file activity span multiple poll cycles. Without kernel-level telemetry (e.g. macOS Endpoint Security Framework, or ESF), **short-burst agentic patterns** (e.g. spawn shell, run git, exit in under 2 seconds) may be missed by the polling fallback. See [docs/esf-entitlement.md](../docs/esf-entitlement.md) for ESF status and implications.
...
| **Identity** | `accountName` and `emailAddress` in cleartext in every session JSON. Observed: `"Evan"`, `"evan@detecadg.com"`. **Strongest identity signal of any profiled tool** — top-level JSON field, zero extraction effort. | High | ✓ Confirmed (new) |
...
Confidence varies by tool and environment. High-risk autonomous tools (e.g. Claude Code) are often reported at **Medium** confidence when process and file layers anchor detection but network/behavioral layers are limited (e.g. without EDR or kernel telemetry). Sales and docs should not imply uniform high confidence across all tools; see [docs/esf-entitlement.md](../docs/esf-entitlement.md) and messaging guidance.
...
event {
  ...
  tool {
    ...
    class:                   enum (A | B | C | D); D = Persistent Autonomous Agent (Section 5.1)
...
| File / Artifact | File integrity monitoring (FIM), EDR file event stream, periodic artifact sweeps | Schedule artifact inventory scans for model storage dirs; hash-based change detection. **OS-level scheduled runs** (user crontab, `/etc/cron.d/`, LaunchAgent plists) are detected via a dedicated scheduler-artifact scan; short cron runs may still be missed behaviorally until ESF/native telemetry is available. |
...
**Scanner platform abstraction:** All listed scanners use the platform abstraction (`collector/compat/`); migration is complete.
...
| Claude Cowork | **Standard install + launch + session analysis + teardown (CW-POS-01)**, MCP connector agentic task, scheduled task exercise (CW-POS-02), DXT chrome-control (CW-POS-03), skill-creator. Gap scenarios: LAB-RUN-015 (RESULTS 2026-03-16; protocol-expected). | Non-standard install, VPN-routed API, DXT extension modification | `IN PROGRESS` (2/3 positive; CW-POS-01 live, CW-POS-02/03/skill-creator validated via LAB-RUN-015) |
...
**From LAB-RUN-014 protocol enhancement (INIT-43 alignment, 2026-03):**

44. **Protocol and capture script alignment to INIT-43 improves repeatability.** LAB-RUN-014 protocol was updated with explicit mapping to INIT-43 process/file/network normalization fields, optional connection and pstree stream monitors for process–network linkage, Phase 5 analysis template (signal matrix, correlation rules C1–C4, confidence trace, residual ambiguity), and evidence inventory checklist. Required outputs (per-layer report, confidence trace, correlation rule evaluation, residual ambiguity) are defined in the protocol and must appear in RESULTS. Executable capture script `lab-runs/scripts/cowork-capture.sh` implements baseline, phase1, phase2, and phase4. LAB-RUN-015 protocol defines gap scenarios CW-POS-02 (scheduled task), CW-POS-03 (DXT chrome-control), and skill-creator with same INIT-43 output requirements.
...
| LAB-RUN-001-root | 2026-03-16 | Claude Code | CC-POS-01: Root rerun with full visibility (tcpdump, process-to-network attribution) | **Pass** | Protocol-expected (Path B). Confidence: 0.825 (High). Network attributable; no unresolved_proc_net_linkage penalty. Fixture: `LAB-RUN-001-root.json`. Full results: `LAB-RUN-001-root-RESULTS.md` |
...
| LAB-RUN-014 | 2026-03-05 | Claude Cowork v1.1.4498 (Claude Desktop) | CW-POS-01: Standard install + launch + session analysis + teardown | **Pass** | **First Claude Cowork validation. New highest confidence score: 0.905 (High) — surpasses OpenClaw (0.80). First tool with VM-based execution.** 12 processes (+ VM XPC), ~546 MB RSS. Key findings: (1) Full Linux VM via Apple Virtualization framework — 10 GB `rootfs.img` is largest artifact of any tool, (2) cleartext `accountName` + `emailAddress` in every session JSON — strongest identity signal, (3) `coworkScheduledTasksEnabled` + `schedule` skill = "soft proactive" execution (Class D-adjacent), (4) DXT extensions enable cross-app automation (Chrome, Apple Notes) on host outside VM sandbox, (5) plugin marketplace with 19+ enterprise plugins, (6) `skill-creator` enables self-modification, (7) ShipIt auto-updater runs as root, (8) VM egress allowlist (22 domains) is novel compliance signal, (9) persistent TLS PID-attributable (like Cursor), (10) feature flags in process args enable version fingerprinting. New Section 4.1b created. Protocol later aligned to INIT-43 with Phase 5 template and evidence checklist; capture script: `lab-runs/scripts/cowork-capture.sh`. Full results: `LAB-RUN-014-RESULTS.md` |
| LAB-RUN-008 | 2026-03-16 | Aider (aider-chat v0.86.2) | AI-POS-01: Standard install + repo editing session | **Pass** | Live run. Repo in ~/Documents/aider-lab. Confidence: 0.5675 (Medium). Process + pip + .aider* in repo + ~/.aider/ + recent artifacts; network 0 at scan time. Full results: `LAB-RUN-008-RESULTS.md` |
| LAB-RUN-012 | 2026-03-16 | Cline (saoudrizwan.claude-dev) | CLINE-POS-01: Install + multi-file edit + Class C tool-call escalation | **Conditional Pass** | Evidence from collector dry-run; Cline extension not installed on lab machine. RESULTS and fixture use protocol-expected scenario. Confidence: 0.62 (Medium) for full scenario; Class C when write/exec or tool_use in latest task. Full results: `LAB-RUN-012-RESULTS.md` |
| LAB-RUN-010 | 2026-03-16 | Continue (continue.continue) | CONT-POS-01: Install + config + Ollama backend + active coding session | **Conditional Pass** | Protocol-expected scenario only (no live run). Confidence: 0.615 (Medium). Class A; Warn for unapproved backend. Config is primary anchor; network often 0 without elevation. Full results: `LAB-RUN-010-RESULTS.md` |
| LAB-RUN-009 | 2026-03-16 | LM Studio | LMS-POS-01: Install + model download + local server mode | **Conditional Pass** | Protocol-expected scenario only (no live run). Confidence: 0.71 (Medium). Class B; local server on :1234 trivially attributable; identity weak (OS user only). Full results: `LAB-RUN-009-RESULTS.md` |
| LAB-RUN-011 | 2026-03-16 | GPT-Pilot | GPT-POS-01: Install + project generation task | **Conditional Pass** | Protocol-expected scenario only (no live run). Confidence: 0.71 (Medium). Class C; behavior layer primary anchor; approval required for R3. Full results: `LAB-RUN-011-RESULTS.md` |
| LAB-RUN-015 | 2026-03-16 | Claude Cowork (Claude Desktop) | CW-POS-02 (scheduled task), CW-POS-03 (DXT chrome-control), skill-creator self-modification | **Pass** | Gap run. Confidence: 0.89 (High), unchanged from LAB-RUN-014. Protocol-expected scenario; evidence supplied. C1 alignment for 3D/3E/3F; residual: scheduled run timing, DXT host attribution, no approval gate for skill-creator. Full results: `LAB-RUN-015-RESULTS.md` |
...
*End of Playbook v0.4.1. Aligned with repo: tool class D, 12 tools, compat migration complete, version refs updated. Prior: LAB-RUN-014 (Claude Cowork).*
