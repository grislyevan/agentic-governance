...
class TestAgentDownloadAuth:
    def test_unauthenticated_returns_401(self, client):
        resp = client.get(f"{API}/agent/download?platform=windows")
        assert resp.status_code == 401

    def test_viewer_role_returns_403(self, client):
        headers, _ = _register_viewer(client)
        resp = client.get(f"{API}/agent/download?platform=windows", headers=headers)
...
class TestAgentDownloadTenantKey:
    def test_zip_contains_tenant_agent_key(self, client, tmp_path):
        ...
        assert cfg.get("protocol") == "http"
        assert "gateway_port" not in cfg
        assert "gateway_host" not in cfg
    ...
    def test_tenant_key_is_auto_generated(self, client, tmp_path):
        ...
        with patch("routers.agent_download._DIST_DIR", pkg_dir):
            resp1 = client.get(f"{API}/agent/download?platform=windows", headers=headers)
            resp2 = client.get(f"{API}/agent/download?platform=windows", headers=headers)
...
class TestTokenDownload:
    def test_token_download_works(self, client, tmp_path):
        ...
        with patch("routers.agent_download._DIST_DIR", pkg_dir):
            resp = client.get(f"{API}/agent/download/{raw_token}?platform=windows")
    ...
    def test_token_single_use(self, client, tmp_path):
        ...
        with patch("routers.agent_download._DIST_DIR", pkg_dir):
            resp1 = client.get(f"{API}/agent/download/{raw_token}?platform=windows")
            resp2 = client.get(f"{API}/agent/download/{raw_token}?platform=windows")
    ...
    def test_invalid_token_returns_400(self, client):
        resp = client.get(f"{API}/agent/download/bogus-token-value?platform=windows")
...
class TestEmailEnrollment:
    def test_enroll_email_requires_auth(self, client):
        resp = client.post(f"{API}/agent/enroll-email", json={
            "email": "user@test.com", "platform": "windows",
        })
        assert resp.status_code == 401
    ...
    def test_enroll_email_smtp_not_configured(self, client):
        headers, _ = _register_owner(client)
        resp = client.post(f"{API}/agent/enroll-email", json={
            "email": "user@test.com", "platform": "linux",
        }, headers=headers)
...
class TestAgentKeyManagement:
    ...
    def test_get_key_after_download(self, client, tmp_path):
        ...
        headers, _ = _register_owner(client)
        with patch("routers.agent_download._DIST_DIR", pkg_dir):
            client.get(f"{API}/agent/download?platform=windows", headers=headers)
        resp = client.get(f"{API}/agent/key", headers=headers)
        assert resp.status_code == 200
        data = resp.json()
...
class TestAgentDownloadValidation:
    ...
    def test_interval_too_low_returns_422(self, client):
        headers, _ = _register_owner(client)
        resp = client.get(f"{API}/agent/download?platform=windows&interval=5", headers=headers)
        assert resp.status_code == 422

    def test_explicit_protocol_auto_includes_gateway(self, client, tmp_path):
        pkg_dir = _create_fake_package(tmp_path, "detec-agent.zip")
        headers, _ = _register_owner(client)
        with patch("routers.agent_download._DIST_DIR", pkg_dir):
            resp = client.get(
                f"{API}/agent/download?platform=windows&protocol=auto", headers=headers
            )
        assert resp.status_code == 200
        cfg = json.loads(zipfile.ZipFile(BytesIO(resp.content)).read("collector.json"))
        assert cfg.get("protocol") == "auto"
        assert "gateway_port" in cfg

    def test_env_default_protocol_auto(self, client, tmp_path):
        pkg_dir = _create_fake_package(tmp_path, "detec-agent.zip")
        headers, _ = _register_owner(client)
        with patch("routers.agent_download._DIST_DIR", pkg_dir):
            with patch(
                "routers.agent_download.settings.agent_download_default_protocol", "auto"
            ):
                resp = client.get(f"{API}/agent/download?platform=windows", headers=headers)
        cfg = json.loads(zipfile.ZipFile(BytesIO(resp.content)).read("collector.json"))
        assert cfg.get("protocol") == "auto"
