...
class TestRegister:
    ...
    def test_register_custom_tenant_name(self, client):
        ...
        me = client.get(f"{API}/auth/me", headers=_auth_header(tokens["access_token"]))
        assert me.status_code == 200

    def test_register_disabled_returns_403(self, client, monkeypatch):
        """P2b: ALLOW_REGISTRATION=false -> 403."""
    ...
    def test_register_enabled_by_default(self, client):
        """P2b: default behavior unchanged — registration works."""
        resp = client.post(
            f"{API}/auth/register",
            json={
...
class TestMe:
    def test_me_returns_user_info(self, client):
        ...
        resp = client.get(
            f"{API}/auth/me", headers=_auth_header(tokens["access_token"])
        )
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_me_with_bad_token_returns_401(self, client):
        resp = client.get(f"{API}/auth/me", headers={"Authorization": "Bearer garbage"})
        assert resp.status_code == 401
