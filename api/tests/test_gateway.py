...
def _seed_tenant_with_agent_key() -> tuple[str, str]:
    """Create a tenant with agent key, return (tenant_id, agent_key)."""
...
class TestAgentSessionAuth:
    ...
    async def test_auth_success(self) -> None:
        _reset_db()
        tenant_id, api_key = _seed_tenant_with_agent_key()
...
class TestGatewayProductionTLSRequirement:
    def test_production_requires_tls_when_gateway_enabled(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from core.server_config import GatewayConfig, require_gateway_tls_for_production
        from core.config import settings

        monkeypatch.setenv("ENV", "production")
        monkeypatch.setattr(settings, "gateway_tls_cert", "")
        monkeypatch.setattr(settings, "gateway_tls_key", "")

        with pytest.raises(RuntimeError, match="Gateway TLS is required"):
            require_gateway_tls_for_production(GatewayConfig(enabled=True, host="0.0.0.0", port=8001))

    def test_production_allows_enabled_gateway_with_tls(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from core.server_config import GatewayConfig, require_gateway_tls_for_production
        from core.config import settings

        monkeypatch.setenv("ENV", "production")
        monkeypatch.setattr(settings, "gateway_tls_cert", "/tmp/cert.pem")
        monkeypatch.setattr(settings, "gateway_tls_key", "/tmp/key.pem")

        require_gateway_tls_for_production(GatewayConfig(enabled=True, host="0.0.0.0", port=8001))

    def test_production_allows_disabled_gateway_without_tls(self, monkeypatch: pytest.MonkeyPatch) -> None:
        from core.server_config import GatewayConfig, require_gateway_tls_for_production
        from core.config import settings
        ...
        require_gateway_tls_for_production(GatewayConfig(enabled=False, host="0.0.0.0", port=8001))
