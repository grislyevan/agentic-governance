...
def _seed_tenant_and_user_api_key() -> tuple[str, str]:
    """Create a tenant and user API key; return (tenant_id, raw_api_key)."""
...
class TestGatewayTenantAgentKeyAuth:
    ...
    @pytest.mark.asyncio
    async def test_user_api_key_rejected_for_gateway_auth(self) -> None:
        _reset_db()
        _tenant_id, user_api_key = _seed_tenant_and_user_api_key()
        ...
        msg = auth_msg(user_api_key, "user-key-host", "0.3.0", seq=1)
        await session.handle_message(msg)

        assert not session.authenticated
        sent = _extract_messages(transport)
        assert sent[0]["t"] == MessageType.AUTH_FAIL
        assert "Invalid API key" in sent[0]["p"].get("reason", "")

    @pytest.mark.asyncio
    async def test_api_key_too_long_rejected(self) -> None:
        _reset_db()
        _seed_tenant_with_agent_key()
        ...
        msg = auth_msg("x" * 257, "agent-key-host", "0.3.0", seq=1)
        await session.handle_message(msg)
