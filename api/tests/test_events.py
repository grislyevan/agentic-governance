...
class TestIngestEvent:
    ...
    def test_ingest_invokes_playbook_background_path(self, client):
        headers = _auth(client)
        with patch("routers.events._run_playbooks_background") as mock_playbooks:
            resp = client.post(f"{API}/events", json=_event_body("pb-evt-1"), headers=headers)
            assert resp.status_code == 201
            # Starlette BackgroundTasks executes before response returns in TestClient.
            assert mock_playbooks.call_count == 1
...
class TestListEvents:
    ...
    def test_list_filter_by_tool_name(self, client):
        headers = _auth(client)
        client.post(f"{API}/events", json=_event_body("f-1", tool={"name": "Cursor"}), headers=headers)
        client.post(f"{API}/events", json=_event_body("f-2", tool={"name": "Ollama"}), headers=headers)
...
class TestGetEvent:
    def test_get_event_by_id_returns_200(self, client):
        headers = _auth(client)
        ingest = client.post(f"{API}/events", json=_event_body("get-ev-1"), headers=headers)
    ...
    def test_get_event_404_for_unknown_id(self, client):
        headers = _auth(client)
        resp = client.get(f"{API}/events/00000000-0000-0000-0000-000000000000", headers=headers)
...
class TestBlockEvent:
    def test_block_event_returns_200_and_creates_admin_block_event(self, client):
        headers = _auth(client)
        ingest = client.post(f"{API}/events", json=_event_body("block-ev-1"), headers=headers)
        assert ingest.status_code in (200, 201)
        event_id = ingest.json()["id"]
        ...
        admin_blocks = [e for e in items if e.get("event_type") == "enforcement.admin_block"]
    ...
    def test_block_event_403_for_analyst(self, client):
        ...
        owner_headers = _auth_header(login_user(client, "block-owner@test.com")["access_token"])
        ingest = client.post(f"{API}/events", json=_event_body("block-ev-2"), headers=owner_headers)
        assert ingest.status_code in (200, 201)
        event_id = ingest.json()["id"]
        ...
        analyst_headers = _auth_header(login_user(client, "block-owner@test.com")["access_token"])
    ...
    def test_block_event_404_for_unknown_id(self, client):
        headers = _auth(client)
        resp = client.post(f"{API}/events/00000000-0000-0000-0000-000000000000/block", headers=headers)
        assert resp.status_code == 404

    def test_block_event_cross_tenant_owner_denied(self, client):
        owner_a = _auth_header(register_user(client, "owner-a-events@test.com", tenant_name="Events A")["access_token"])
        owner_b_tokens = register_user(client, "owner-b-events@test.com", tenant_name="Events B")
        owner_b = _auth_header(owner_b_tokens["access_token"])

        ingest = client.post(f"{API}/events", json=_event_body("x-tenant-block-1"), headers=owner_b)
        assert ingest.status_code in (200, 201)
        event_id = ingest.json()["id"]
