...
def test_demo_session_returns_canned_report(client):
    """GET /demo/session returns a canned session report (auth required)."""
    headers = _setup_owner(client)
    resp = client.get(f"{API}/demo/session", headers=headers)
    assert resp.status_code == 200
    data = resp.json()
    assert data["id"] == "demo-session-canned"
    assert data["tool"] == "Claude Code"
    assert data.get("session_timeline")
    assert len(data["session_timeline"]) >= 4
    assert data.get("session_verdict") == "risky"
    assert data.get("key_evidence")
