...
def require_gateway_tls_for_production(gw_cfg: GatewayConfig) -> None:
    """Raise if gateway is enabled in production/staging without cert+key."""
    ...
    if settings.gateway_tls_cert and settings.gateway_tls_key:
        return
    raise RuntimeError(
        "Gateway TLS is required in production/staging when the gateway is enabled. "
        "Set both GATEWAY_TLS_CERT and GATEWAY_TLS_KEY, or disable the gateway."
    )
