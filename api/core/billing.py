...
def _validate_return_url(url: str, allowed_origins: list[str]) -> None:
    """Raise ValueError if url's origin is not in allowed_origins."""
    if not allowed_origins:
        return  # not configured — allow all (backward compat)
    parsed = urlparse(url)
    origin = f"{parsed.scheme}://{parsed.netloc}"
    if origin not in allowed_origins:
        raise ValueError(
            f"Return URL origin {origin!r} is not in ALLOWED_RETURN_ORIGINS. "
...
def create_checkout_session(
    ...
    _init_stripe()
    allowed = settings.allowed_return_origins_list
    _validate_return_url(success_url, allowed)
    _validate_return_url(cancel_url, allowed)
...
def create_portal_session(tenant_id: str, return_url: str) -> str:
    ...
    _init_stripe()
    _validate_return_url(return_url, settings.allowed_return_origins_list)
    from core.database import SessionLocal
    from models.tenant import Tenant
