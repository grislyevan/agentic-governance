...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    def _make_task_supervisor(name: str):
        def _cb(t: asyncio.Task):
            if not t.cancelled() and t.exception():
                logger.error("Background task %s crashed: %s", name, t.exception(), exc_info=t.exception())
        return _cb

    monitor_task = asyncio.create_task(background_tasks.staleness_monitor())
    monitor_task.add_done_callback(_make_task_supervisor("staleness_monitor"))

    purge_task = asyncio.create_task(background_tasks.retention_purge_loop())
    purge_task.add_done_callback(_make_task_supervisor("retention_purge_loop"))
    ...
    try:
        ...
        require_gateway_tls_for_production(gw_cfg)
    finally:
        db.close()
    if gw_cfg.enabled:
