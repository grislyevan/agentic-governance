...
from core.tenant import get_tenant_filter, resolve_auth, require_role, strict_tenant_filter
...
def get_profile(
    ...
    profile = (
        db.query(EndpointProfile)
        .filter(EndpointProfile.id == profile_id, strict_tenant_filter(auth, EndpointProfile))
        .first()
    )
    if not profile:
