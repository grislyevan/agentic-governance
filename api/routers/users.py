...
@router.get("/me/api-key", response_model=MyApiKeyStatusOut)
def get_my_api_key_status(
    ...
    return MyApiKeyStatusOut(
        prefix_display=display,
        configured=bool(user.api_key_hash),
    )


@router.post("/me/api-key/rotate", response_model=RotateApiKeyOut)
def rotate_my_api_key(
    ...
    db.commit()
    logger.info("User %s rotated API key", user.id)
    return RotateApiKeyOut(api_key=raw)
