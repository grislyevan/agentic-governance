...
from core.demo_session import get_canned_demo_session
from models.endpoint import Endpoint
from models.event import Event
from schemas.session_report import SessionReport
...
@router.get("/session", response_model=SessionReport)
def demo_session(
    authorization: str | None = Depends(get_authorization),
    x_api_key: str | None = Header(default=None),
    db: Session = Depends(get_db),
) -> SessionReport:
    """Return a canned session report for demos when the session list is empty.
    Authenticated; no demo_mode required so the dashboard can show at least one session.
    """
