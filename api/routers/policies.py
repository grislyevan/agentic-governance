...
class CreatePolicyFromEventBody(BaseModel):
    event_id: str = Field(description="Event id (uuid from events list/detail)")
...
def create_policy_from_event(
    request: Request,
    body: CreatePolicyFromEventBody,
    db: Session = Depends(get_db),
    authorization: str | None = Depends(get_authorization),
    x_api_key: str | None = Header(default=None),
) -> PolicyResponse:
    """Create a block policy from an event so similar detections are blocked going forward. Admin or owner only."""
    auth = resolve_auth(authorization, x_api_key, db)
    require_role(auth, "owner", "admin")
    ...
    sensitivity_tier = target.get("sensitivity_tier") or target.get("sensitivity_tiers")
    if isinstance(sensitivity_tier, list):
        sensitivity_tiers = sensitivity_tier
    elif sensitivity_tier:
        sensitivity_tiers = [sensitivity_tier]
    else:
        sensitivity_tiers = ["Tier0", "Tier1", "Tier2", "Tier3"]

    rule_id = f"CUSTOM-BLOCK-{uuid.uuid4().hex[:8].upper()}"
    description = (
        f"Block {tool_name} (created from event). "
        f"Tool class: {tool_class}. Applies to similar detections going forward."
    )
    parameters = {
        "decision_state": "block",
        "conditions": {
            "tool_classes": [tool_class] if tool_class else ["A", "B", "C"],
