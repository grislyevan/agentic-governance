...
class AgentSession(BaseConnection):
    ...
    async def _read_loop(self) -> None:
        ...
        while not self._closed:
            try:
                ...
                # Decode error (e.g. frame size, msgpack) or transport read error.
                detec_gateway_decode_errors_total.inc()
                logger.warning("%s: decode/read error, closing: %s", self._label, e)
                break
            ...
            try:
                ...
                # Per-message handler failure; session continues (fail-open for other messages).
    ...
    async def _handle_auth(self, msg: dict[str, Any]) -> None:
        ...
        if len(api_key) > _MAX_API_KEY_LEN:
            await self.send(auth_fail_msg("API key too long"))
            await self.close()
            return
        ...
        result = await asyncio.to_thread(self._do_auth_db, api_key, hostname)
        if result is None:
            await self.send(auth_fail_msg("Internal authentication error"))
            await self.close()
            return
        tenant_id, endpoint_id = result
    ...
    def _do_auth_db(self, api_key: str, hostname: str) -> tuple[str, str] | None:
        """Open a DB session, verify the API key, and find/create the endpoint.

        Returns (tenant_id, endpoint_id) on success, or a tuple where tenant_id
        is None when the key is invalid, or None on unexpected DB failure.
        Runs entirely in a thread via asyncio.to_thread so no DB I/O touches
        the event loop.
        """
        ...
        try:
            ...
            logger.exception("Auth error for hostname %s (integrity)", hostname)
            return None
        except Exception:
            db.rollback()
            detec_gateway_db_errors_total.inc()
...
class DetecGateway:
    ...
    async def _handle_connection(
        ...
        try:
            ...
            logger.debug("Session transport closed for %s", peer)
        except Exception:
            detec_gateway_transport_errors_total.inc()
    ...
    async def stop(self) -> None:
        ...
        # 1. Close active sessions first for clean agent disconnects
        ...
        if self._server:
            ...
            try:
                ...
                logger.warning("Gateway stop() timed out waiting for server to close")
    ...
    async def push_kill_process(
        ...
        """Push a kill_process command to a specific connected agent.

        Returns True if the agent was connected and the command was sent,
        False if the agent is not connected (offline or using HTTP emitter).
        """
        session = await self._registry.get(endpoint_id)
        if not session:
            logger.warning(
                "push_kill_process: endpoint %s not connected via TCP, kill command not sent",
                endpoint_id,
            )
            return False
        msg = kill_process_msg(
            pid=pid,
        ...
        try:
            ...
            logger.warning(
                "push_kill_process: connection error sending kill command to endpoint %s",
                endpoint_id,
            )
            return False
