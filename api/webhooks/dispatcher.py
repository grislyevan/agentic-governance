...
from webhooks.matching import _matches
    ...
    for wh in webhooks:
        if _matches(wh.events, event_type, decision_state):
            ...
            try:
                ...
                asyncio.run_coroutine_threadsafe(deliver(wh.url, wh.secret, payload, extra_headers), loop)
            except RuntimeError:
                logger.debug("No event loop available; skipping async webhook delivery")
            matched += 1
