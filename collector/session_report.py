...
class SessionReportData:
    ...
    evasion_vectors: list[str] | None = None  # E1–E8 vector IDs and/or capability_drift (E3-05)
...
def build_session_reports(
    ...
    for i, scan in enumerate(detected_scans):
        ...
        evidence = scan.evidence_details or {}
        evasion_findings = evidence.get("evasion_findings") or []
        evasion_vectors = (
            [f["vector"] for f in evasion_findings if isinstance(f, dict) and f.get("vector")]
            if evasion_findings else None
        )
        reports.append(
            SessionReportData(
                tool=tool_name,
                ...
                timeline=timeline,
                evasion_vectors=evasion_vectors,
            )
        )
...
def format_session_report_for_cli(report: SessionReportData) -> str:
    ...
    if report.risk_signals:
        ...
        lines.append("- (none)")
    if getattr(report, "evasion_vectors", None):
        lines.append("")
        lines.append("evasion/tamper vectors:")
        for v in report.evasion_vectors:
            lines.append(f"- {v}")
