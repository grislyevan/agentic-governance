...
`protocol` selects the transport: `auto` (default, TCP first with HTTP fallback), `tcp` (TcpEmitter only), or `http` (HttpEmitter only). For `auto` or `tcp`, `gateway_host` defaults to the hostname from `api_url` and `gateway_port` defaults to `8001`. Optional JSON keys `tcp_host` / `tcp_port` alias `gateway_host` / `gateway_port` when the latter are omitted. Tune auto mode with `tcp_connect_timeout_seconds`, `tcp_retry_interval_seconds`, `tcp_failure_threshold`, and `tcp_recovery_stability_seconds` (see `AGENTIC_GOV_TCP_*` in `config_loader.py`).
