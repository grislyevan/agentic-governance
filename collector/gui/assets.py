"""GUI assets for the Detec Agent.

Menu bar: prefers ``branding/Icon-menubar.icns`` (monochrome template, matches
other menu bar icons in light/dark mode). Otherwise ``branding/Icon.icns``
(full color, shown as-is). Fallback: generated template PNG.

Optional ``Icon-menubar.icns``: black or gray artwork on transparent background;
system tints it. Full-color ``Icon.icns`` uses non-template mode so it does not
look inverted.
"""
...
def get_menubar_icon_path() -> Path:
    """Return path to the menu bar icon file.

    Resolution order:
    1. ``branding/Icon-menubar.icns`` (monochrome template; best for menu bar)
    2. ``branding/Icon.icns`` (full color; use when no menubar-specific asset)
    3. PyInstaller ``icons/menubar-template.png`` or generated cache (legacy)
    """
    bundle_dir = getattr(sys, "_MEIPASS", None)
    for name in ("Icon-menubar.icns", "Icon.icns"):
        if bundle_dir:
            p = Path(bundle_dir) / "branding" / name
            if p.is_file():
                return p
        p = _repo_root() / "branding" / name
        if p.is_file():
            return p
...
def menubar_icon_uses_template(icon_path: Path) -> bool:
    """macOS template images must be monochrome; full-color .icns needs template off."""
    name = icon_path.name.lower()
    if name == "icon-menubar.icns":
        return True
    if name == "icon.icns":
        return False
    return True
