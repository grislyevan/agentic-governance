...
class DaemonBridge:
    ...
    def _run(self, config: dict[str, Any]) -> None:
        ...
        gateway_port = int(config.get("gateway_port", 8001) or 8001)
            ...
            if protocol == "auto":
                emitter = AdaptiveEmitter(
                    ...
                    tcp_connect_timeout=float(
                        config.get("tcp_connect_timeout_seconds", 3.0) or 3.0
                    ),
                    tcp_retry_interval=float(
                        config.get("tcp_retry_interval_seconds", 30.0) or 30.0
                    ),
                    tcp_failure_threshold=int(
                        config.get("tcp_failure_threshold", 5) or 5
                    ),
                    tcp_recovery_stability=float(
                        config.get("tcp_recovery_stability_seconds", 10.0) or 10.0
                    ),
                )
            elif protocol == "tcp":
                emitter = TcpEmitter(
        ...
        if protocol == "auto":
            transport = f"auto (tcp→http) {gateway_host}:{gateway_port}"
        elif protocol == "tcp":
            transport = f"tcp://{gateway_host}:{gateway_port}"
        else:
            transport = api_url
        logger.info(
            "DaemonBridge: started (interval=%ds, transport=%s)",
            interval,
            transport,
        )
        ...
        try:
            ...
            try:
                ...
                logger.debug("DaemonBridge: emitter shutdown", exc_info=True)
