...
def test_push_and_query_file_read_events() -> None:
    store = EventStore(max_events=100, retention_seconds=60.0)
    now = _utc_now()
    ...
    assert len(events) == 1
    assert events[0].path == "/etc/hosts"
...
def test_get_event_counts() -> None:
    store = EventStore(max_events=100, retention_seconds=60.0)
    assert store.get_event_counts() == {"process": 0, "network": 0, "file": 0, "file_read": 0}
    now = _utc_now()
    store.push_process(
        ProcessExecEvent(timestamp=now, pid=1, ppid=0, name="a", cmdline="a", source="polling")
    )
    store.push_network(
        NetworkConnectEvent(
            timestamp=now, pid=1, process_name="a", remote_addr="x", remote_port=443,
            local_port=1, source="polling",
        )
    )
    store.push_file(FileChangeEvent(timestamp=now, path="/x", action="modified", source="polling"))
    counts = store.get_event_counts()
    assert counts["process"] == 1
    assert counts["network"] == 1
    assert counts["file"] == 1
    assert counts["file_read"] == 0


def test_rate_cap_drops_when_exceeded() -> None:
    store = EventStore(
        max_events=100,
        retention_seconds=60.0,
        rate_cap_per_second=2,
    )
    now = _utc_now()
    for i in range(5):
        store.push_process(
...
def test_rate_cap_disabled_by_default() -> None:
    store = EventStore(max_events=100, retention_seconds=60.0)
    now = _utc_now()
    for i in range(20):
        store.push_process(
    ...
    assert len(store.get_process_events()) == 20
