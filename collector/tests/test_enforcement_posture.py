...
class TestEnforcerPosture:
    ...
    def test_active_network_block_schedules_ttl_unblock(self, tmp_path: Path) -> None:
        mgr = PostureManager(initial_posture="active", state_dir=tmp_path)
        enforcer = Enforcer(posture_manager=mgr, network_block_ttl_seconds=60)

        with patch("enforcement.network_block.block_outbound", return_value=True) as m_block:
            with patch.object(enforcer, "_schedule_network_unblock") as m_sched:
                result = enforcer.enforce(
                    decision=_make_decision("block"),
                    tool_name="TestTool",
                    tool_class="C",
                ...
                m_sched.assert_called_once_with({999}, 60)
    ...
    def test_detect_warn_always_log_without_proxy(self, tmp_path: Path) -> None:
        mgr = PostureManager(initial_posture="active", state_dir=tmp_path)
        enforcer = Enforcer(posture_manager=mgr)
    ...
    def test_detect_warn_use_proxy_when_configured(self, tmp_path: Path) -> None:
        mgr = PostureManager(initial_posture="active", state_dir=tmp_path)
        enforcer = Enforcer(
            posture_manager=mgr,
            proxy_https="http://proxy.local:8080",
        )

        with patch("enforcement.proxy_inject.inject_proxy_env", return_value=True) as m_inject:
            for state in ("detect", "warn"):
                result = enforcer.enforce(
                    decision=_make_decision(state),
                    tool_name="TestTool",
                ...
                assert result.tactic == "proxy_inject"
                assert result.success is True
            assert m_inject.call_count == 2
    ...
    def test_approval_required_uses_proxy_when_configured(self, tmp_path: Path) -> None:
        mgr = PostureManager(initial_posture="active", state_dir=tmp_path)
        enforcer = Enforcer(
            posture_manager=mgr,
    ...
    def test_block_without_pids_uses_proxy_fallback_when_configured(self, tmp_path: Path) -> None:
        mgr = PostureManager(initial_posture="active", state_dir=tmp_path)
        enforcer = Enforcer(
            posture_manager=mgr,
            proxy_https="http://proxy.local:8443",
        )
