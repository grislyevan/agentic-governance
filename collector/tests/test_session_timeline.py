...
def test_timeline_deterministic_same_store() -> None:
    """Identical event sets produce identical timelines (timestamp + insertion_index)."""
    store = EventStore(max_events=100, retention_seconds=120.0)
    pid = 100
    t = _ts_offset(60)
    store.push_process(
    ...
    result1 = build_session_timeline(store, "Tool", {pid}, expand_tree=False)
    result2 = build_session_timeline(store, "Tool", {pid}, expand_tree=False)
    assert result1 == result2
    assert len(result1) >= 2


def test_timeline_same_timestamp_ordered_by_insertion() -> None:
    """Events with the same timestamp are ordered by insertion index (process, then network, then file)."""
    store = EventStore(max_events=100, retention_seconds=120.0)
    pid = 100
    t = _ts_offset(50)
    store.push_process(
    ...
    assert "file_write" in types or "file_modified" in types
    idx_shell = next(i for i, e in enumerate(result) if e["type"] == "shell_exec")
    idx_file = next(
        i for i, e in enumerate(result)
        if e["type"] in ("file_write", "file_modified")
    )
    assert idx_shell < idx_file, "process (shell) should appear before file at same timestamp"
