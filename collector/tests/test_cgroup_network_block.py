...
class TestBlockLinuxIntegration:
    ...
    def test_no_uid_fallback_when_disabled(self) -> None:
        """When allow_uid_fallback=False (default), do not use iptables UID path."""
        with mock.patch(
            ...
            result = _block_linux({2000}, allow_uid_fallback=False)

        assert result is False
        m_run.assert_not_called()

    def test_falls_back_to_uid_when_cgroup_unavailable_and_allowed(self) -> None:
        with mock.patch(
            "enforcement.network_block._cgroup_v2_block", return_value=False
        ), mock.patch(
            "enforcement.network_block._get_uid_linux", return_value=1001
            ...
            m_run.return_value = mock.Mock(returncode=0, stderr="")
            result = _block_linux({2000}, allow_uid_fallback=True)
    ...
    def test_multiple_pids_mixed(self) -> None:
        ...
        with mock.patch(
            ...
            m_run.return_value = mock.Mock(returncode=0, stderr="")
            result = _block_linux({100, 200}, allow_uid_fallback=True)
