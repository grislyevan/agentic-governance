...
class TestFullScan:
    def test_clean_system_no_evasion(self, scanner):
        with (
            ...
            patch.object(scanner, "_check_agent_interruption_patterns", return_value=[]),
            patch.object(scanner, "_check_config_tamper", return_value=[]),
            patch.object(scanner, "_check_telemetry_shaping", return_value=[]),
        ):
...
class TestE6AgentInterruption:
    """E6: Agent disable/kill loop detection."""

    def test_e6_no_event_store_returns_empty(self, scanner):
        scanner._event_store = None
        findings = scanner._check_agent_interruption_patterns(verbose=False)
        assert findings == []

    def test_e6_kill_loop_detected(self, scanner):
        ...
        findings = scanner._check_agent_interruption_patterns(verbose=False)
        assert len(findings) == 1
        assert findings[0].vector == "E6-agent-kill-loop"
        assert findings[0].boost == 0.12

    def test_e6_single_kill_not_flagged(self, scanner):
        store = EventStore(max_events=100, retention_seconds=400)
        now = datetime.now(timezone.utc)
        store.push_process(
...
class TestE7ConfigTamper:
    """E7: Config tamper / policy suppression."""

    def test_e7_empty_api_url_with_api_key_flagged(self, scanner, tmp_path):
        ...
        with patch("scanner.evasion._collector_config_paths", return_value=[config_path]):
            findings = scanner._check_config_tamper(verbose=False)
        assert len(findings) >= 1
        assert any(f.vector == "E7-config-tamper" for f in findings)

    def test_e7_config_tamper_dry_run_with_api(self, scanner, tmp_path):
        config_path = tmp_path / "collector.json"
        config_path.write_text(json.dumps({"dry_run": True, "api_url": "https://api.example.com"}))
        with patch("scanner.evasion._collector_config_paths", return_value=[config_path]):
            findings = scanner._check_config_tamper(verbose=False)
        assert len(findings) >= 1
        assert any(f.vector == "E7-config-tamper" for f in findings)

    def test_e7_normal_config_not_flagged(self, scanner, tmp_path):
        config_path = tmp_path / "collector.json"
        config_path.write_text(json.dumps({"api_url": "https://api.example.com", "api_key": "x"}))
        with patch("scanner.evasion._collector_config_paths", return_value=[config_path]):
            findings = scanner._check_config_tamper(verbose=False)
        assert len(findings) == 0


class TestE8TelemetryShaping:
    """E8: Telemetry shaping (provider downgrade/suppression)."""

    def test_e8_drift_with_polling_emits_finding(self, scanner):
        result = scanner.scan(
            verbose=False,
            diagnostics_context={
        ...
        assert result.detected
        vectors = [f["vector"] for f in result.evidence_details.get("evasion_findings", [])]
        assert "E8-telemetry-shaping" in vectors

    def test_e8_no_drift_returns_no_e8(self, scanner):
        with (
            patch.object(scanner, "_check_global_hooks", return_value=[]),
            patch.object(scanner, "_check_template_hooks", return_value=[]),
            patch.object(scanner, "_check_repo_hooks", return_value=[]),
            ...
            result = scanner.scan(
                verbose=False,
                diagnostics_context={"capability_drift": [], "provider_name": "polling"},
            )
        assert not result.detected or "E8-telemetry-shaping" not in [
            f["vector"] for f in result.evidence_details.get("evasion_findings", [])
        ]

    def test_e8_esf_provider_with_drift_no_e8(self, scanner):
        with (
            patch.object(scanner, "_check_global_hooks", return_value=[]),
            patch.object(scanner, "_check_template_hooks", return_value=[]),
            patch.object(scanner, "_check_repo_hooks", return_value=[]),
        ...
        vectors = [f["vector"] for f in result.evidence_details.get("evasion_findings", [])]
        assert "E8-telemetry-shaping" not in vectors
