    ...
    def _on_command(command: str, command_id: str, params: dict) -> None:
        if command == "restore_services":
            ...
            logger.info(
                ...
                command_id, pid, process_name, tool_name,
            )
            if pid is not None:
                try:
                    import psutil
                    ...
                    if result.success:
                        logger.info(
                            ...
                            result.detail, pid, result.killed_pids,
                        )
                    else:
                        logger.warning(
                    ...
                    logger.exception("kill_process command raised an exception (pid=%s)", pid)
            else:
    ...
    if protocol == "auto":
        ...
        emitter = AdaptiveEmitter(
            ...
            tcp_failure_threshold=int(
                getattr(args, "tcp_failure_threshold", 5) or 5
            ),
...
def main() -> None:
    ...
    parser.add_argument(
        "--protocol", choices=["auto", "http", "tcp"], default="auto",
    ...
    parser.add_argument(
        "--tcp-connect-timeout", dest="tcp_connect_timeout_seconds", type=float,
        metavar="SEC", default=None,
        help="TCP auth probe timeout for protocol auto (default: 3)",
    )
    parser.add_argument(
        "--tcp-retry-interval", dest="tcp_retry_interval_seconds", type=float,
        metavar="SEC", default=None,
        help="Interval between TCP recovery probes when on HTTP (default: 30)",
    )
    parser.add_argument(
        "--tcp-failure-threshold", dest="tcp_failure_threshold", type=int,
        metavar="N", default=None,
        help="Consecutive TCP reconnect failures before HTTP fallback (default: 5)",
    )
    parser.add_argument(
        "--tcp-recovery-stability", dest="tcp_recovery_stability_seconds", type=float,
        metavar="SEC", default=None,
