...
# E3-03: Minimum response floor for tamper/evasion vectors (INIT-31).
# When agent_status.tamper_vectors is present, policy must not fall below this floor.
# High-impact tamper (E6 kill loop, E7 config, E8 telemetry shaping, capability_drift) -> approval_required.
# Other evasion (E1-E5 trailer/binary/settings) -> warn.
TAMPER_VECTOR_MINIMUM_RESPONSE: dict[str, str] = {
    "E6-agent-kill-loop": "approval_required",
    "E7-config-tamper": "approval_required",
    "E8-telemetry-shaping": "approval_required",
    "capability_drift": "approval_required",
    "E1-global-hook": "warn",
    "E1-repo-hook": "warn",
    "E2-template-hook": "warn",
    "E3-force-push": "warn",
    "E4-renamed-binary": "warn",
    "E5-cursor-git-disabled": "warn",
    "E5-cursor-telemetry-off": "warn",
}


def minimum_decision_for_tamper_vectors(
    tamper_vectors: list[str] | None,
) -> str:
    """Return the minimum decision_state (by severity) for the given tamper/evasion vectors.

    Used to apply a response floor when agent_status.tamper_vectors is present.
    """
    if not tamper_vectors:
        return "detect"
    floors = [TAMPER_VECTOR_MINIMUM_RESPONSE.get(v, "detect") for v in tamper_vectors]
    return max(floors, key=lambda s: _SEVERITY_ORDER.get(s, 0))


def apply_tamper_floor(decision: PolicyDecision, tamper_vectors: list[str]) -> PolicyDecision:
    """Elevate decision to at least the tamper minimum floor; return new decision if elevated."""
    if not tamper_vectors:
        return decision
    floor = minimum_decision_for_tamper_vectors(tamper_vectors)
    curr = _SEVERITY_ORDER.get(decision.decision_state, 0)
    need = _SEVERITY_ORDER.get(floor, 0)
    if need <= curr:
        return decision
    return PolicyDecision(
        decision_state=floor,
        rule_id=decision.rule_id,
        rule_version=decision.rule_version,
        reason_codes=decision.reason_codes + ["tamper_evasion_floor_" + floor],
