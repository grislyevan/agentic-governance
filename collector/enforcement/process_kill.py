...
def kill_process_tree(
    ...
    try:
        ...
        return _kill_process_tree_fallback(pid, expected_pattern, expected_patterns, grace_period)
    ...
    patterns = [p for p in (expected_patterns or []) if p]
    if expected_pattern:
        patterns.append(expected_pattern)
    ...
    if patterns:
        ...
        if not any(p.lower() in cmdline_l for p in patterns):
            return KillResult(
                pid=pid,
                success=False,
