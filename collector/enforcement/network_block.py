...
def block_outbound(pids: set[int], allow_uid_fallback: bool = False) -> bool:
    """Block outbound connections for the given PIDs.

    Returns True if at least one blocking rule was successfully applied.

    On Linux, when cgroup v2 net_cls is unavailable, blocking can fall back
    to iptables --uid-owner, which affects ALL processes for that UID (high
    collateral). Pass allow_uid_fallback=True only when explicitly enabled
    (e.g. via config allow_linux_uid_block_fallback). See docs/enforcement-safety-matrix.md.
    """
    system = platform.system()
    if system == "Darwin":
        return _block_macos(pids)
    if system == "Linux":
        return _block_linux(pids, allow_uid_fallback=allow_uid_fallback)
    if system == "Windows":
...
def _block_linux(pids: set[int], allow_uid_fallback: bool = False) -> bool:
    ...
    False, we fall back to blocking by UID only when allow_uid_fallback is True.
    UID blocking affects ALL processes owned by that UID (high collateral).
    """
    success = False
    for pid in pids:
        if _cgroup_v2_block(pid):
            success = True
            continue
        if not allow_uid_fallback:
            logger.warning(
                "Skipping UID fallback for PID %d (allow_linux_uid_block_fallback=false). "
                "Set allow_linux_uid_block_fallback in config or AGENTIC_GOV_ALLOW_LINUX_UID_BLOCK_FALLBACK=1 to enable.",
                pid,
            )
            continue
