...
def build_trees(store: EventStore) -> list[ProcessNode]:
    ...
    file_read_by_pid: dict[int, list[FileReadEvent]] = {}
    for e in store.get_file_read_events():
        if e.pid is not None:
            file_read_by_pid.setdefault(e.pid, []).append(e)
    ...
    for pid, e in latest_by_pid.items():
        nodes[pid] = ProcessNode(
            ...
            file_read_events=file_read_by_pid.get(pid, []),
