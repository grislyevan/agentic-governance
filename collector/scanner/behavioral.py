...
from .tree_attribution import classify_tree, get_unknown_roots

if TYPE_CHECKING:
    from telemetry.event_store import EventStore

from telemetry.capabilities import TelemetryCapabilities, get_capabilities_from_store
...
class BehavioralScanner(BaseScanner):
    ...
    def scan(
        self,
        verbose: bool = False,
        capabilities_override: TelemetryCapabilities | None = None,
    ) -> ScanResult:
        result = ScanResult(
            detected=False,
        ...
        for tree in trees:
            classification = classify_tree(tree, self._exclude_pids)
            if classification == "named_only":
                self._log(
                    f"Skipping tree rooted at PID {tree.pid} (named_only)",
                    verbose,
                )
                continue
            if classification == "unknown_only":
                ...
                continue
            unknown_roots = get_unknown_roots(tree, self._exclude_pids)
            if not unknown_roots:
                self._log(
                    f"Skipping mixed tree rooted at PID {tree.pid} (no unknown-only branches)",
                    verbose,
                )
                continue
            for root in unknown_roots:
                candidate_trees.append(root)
        ...
        result.evidence_details = self._build_evidence(
            best_matches, best_tree, capabilities=capabilities,
        )
        result.process_patterns = [best_tree.name]

        # Upgrade to class D if resurrection pattern matched
        for m in best_matches:
            if m.pattern_id == "BEH-008" and m.score >= 0.8:
                result.tool_class = "D"
                break
    ...
    def _aggregate_score(self, matches: list[PatternMatch]) -> float:
        ...
        Normalizes by the sum of weights for ACTIVE (matched) patterns only,
        not the total weight of all 9 possible patterns.  Dividing by all
        possible weights caused valid single-pattern detections to be diluted
        below the 0.45 Medium threshold even when the pattern fired strongly.
        ...
        # Normalize by active pattern weights only, not max possible.
        # This prevents single strong detections from being suppressed by
        # the weight of non-firing patterns.
        active_weight = sum(pattern_weights.get(m.pattern_id, 1.0) for m in matches)
        if active_weight == 0:
            return 0.0
    ...
    def _build_evidence(
        ...
        *,
        ...
        if capabilities is not None:
            gaps = capabilities.gaps_affecting_detection()
            if gaps:
                out["telemetry_notes"] = (
                    "Detection may be limited: missing telemetry for "
                    + ", ".join(gaps)
                )
        return out
