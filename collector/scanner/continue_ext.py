...
class ContinueScanner(BaseScanner):
                    ...
                    if re.match(r"continue\.continue-", entry.name, re.IGNORECASE):
                        ...
                        if pkg.is_file():
                            try:
                                ...
                                if version:
                                    self._log(
                                        f"Version from extension manifest: {version}",
                                        verbose,
                                    )
                                    return str(version)
                            except (json.JSONDecodeError, OSError) as exc:
                                logger.debug(
                                    "Could not read Continue package.json %s: %s",
                                    pkg,
                                    exc,
                                )
    ...
    def _scan_process(self, result: ScanResult, verbose: bool) -> float:
        ...
        if strength > 0 and result.evidence_details.get("ide_extension_hosts"):
            self._log(
                f"  {len(result.evidence_details['ide_extension_hosts'])} extension host(s)",
                verbose,
            )
        else:
            ...
            try:
                for entry in ext_dir.iterdir():
                    ...
                    if re.match(r"continue\.continue-", entry.name, re.IGNORECASE):
                        ...
                        self._log(
                            f"  Extension found: {entry.name} ({ide_label})", verbose
                        )
            except (PermissionError, OSError):
                continue
        ...
        for storage_root in _storage_roots():
            ...
            if storage.is_dir():
                try:
                    ...
                    logger.debug(
                        "Could not list Continue globalStorage %s: %s", storage, exc
                    )
    ...
    def _analyze_config(
        ...
        unapproved = [
            b for b in backends_found if any(u in b for u in KNOWN_UNAPPROVED_BACKENDS)
        ]
        if unapproved:
            result.evidence_details["unapproved_backends"] = unapproved
            result.evasion_boost += 0.10
            self._log(
                f"  RISK: Unapproved backend(s): {', '.join(unapproved)}", verbose
            )
    ...
    def _scan_network(self, result: ScanResult, verbose: bool) -> float:
        ...
        lsof is not called directly — it is Unix-only and not available on Windows.
        """
        ...
        if not ext_host_pids_raw:
            self._log("  No extension host PIDs to check", verbose)
            return strength

        # PIDs may be stored as strings (from pgrep output) or ints; normalise to int.
        try:
            ext_host_pids: set[int] = {int(p) for p in ext_host_pids_raw}
        ...
        for conn in conns:
            if conn.status != "ESTABLISHED":
                continue
            entry = f"{conn.local_addr}:{conn.local_port} -> {conn.remote_addr}:{conn.remote_port}"
            if conn.remote_port == 443:
                tls_conns.append(entry)
            if conn.remote_port in (1234, 11434):  # LM Studio / Ollama
                local_conns.append(entry)
                self._log(
                    "  Local model server connection from extension host", verbose
                )
        ...
        if local_conns:
            ...
            self._log(
                f"  {len(local_conns)} connection(s) to local model server", verbose
            )
        elif tls_conns:
            strength = 0.40
    ...
    def _scan_behavior(self, result: ScanResult, verbose: bool) -> float:
        ...
        if CONTINUE_DIR.is_dir():
            try:
                ...
                logger.debug(
                    "Could not list recent files in ~/.continue/ for behavior scan: %s",
                    exc,
                )
    ...
    def _determine_action(self, result: ScanResult) -> None:
        ...
        result.action_summary = (
            "; ".join(summaries) if summaries else "No Continue signals detected"
        )
