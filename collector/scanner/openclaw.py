...
class OpenClawScanner(BaseScanner):
    ...
    def _scan_identity(self, result: ScanResult, verbose: bool) -> float:
        ...
        # Only treat ambient API keys as a signal when corroborating file evidence
        # exists (data dir present). Generic env vars are not OpenClaw-specific and
        # must not trigger detection in isolation.
        if api_keys_found and self._paths.data_dir and self._paths.data_dir.is_dir():
            strength = max(strength, 0.60)
