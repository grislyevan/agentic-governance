...
Push-Location "$RepoRoot\installers\windows"
pyinstaller --clean --noconfirm detec-server.spec
Pop-Location

# Step 4: Verify output.
$ExePath = "$RepoRoot\installers\windows\dist\detec-server\detec-server.exe"
if (Test-Path $ExePath) {
    $size = (Get-Item $ExePath).Length / 1MB
    Write-Host "`n=== Build complete ===" -ForegroundColor Green
    Write-Host "Executable: $ExePath ($([math]::Round($size, 1)) MB)"
    Write-Host "`nTest it:"
    Write-Host "  cd $RepoRoot\installers\windows\dist\detec-server"
