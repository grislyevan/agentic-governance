...
This runs the full pipeline: dashboard build, server PyInstaller bundle, agent PyInstaller bundle + zip, branding asset generation, and Inno Setup compilation. Output: `installers/windows/dist/DetecServerSetup-0.1.0.exe` and `DetecAgentSetup-0.3.0.exe`.

The server installer bundles the Windows agent package (`detec-agent.zip`) inside `dist/packages/` so agent downloads work from the dashboard immediately after install.
...
irm https://raw.githubusercontent.com/grislyevan/agentic-governance/main/installers/windows/bootstrap.ps1 | iex
```
...
```powershell
Set-ExecutionPolicy Bypass -Scope Process
cd C:\Detec\src\installers\windows
.\deploy.ps1
```
...
This produces `installers/windows/dist/detec-server/` containing `detec-server.exe` and all dependencies.
...
This produces `installers/windows/dist/detec-agent/` containing `detec-agent.exe`.
...
This produces `installers/windows/dist/detec-agent-gui/` containing `detec-agent-gui.exe`. The GUI provides a system tray icon and status window. Double-click to launch; right-click the tray icon for scan controls and quit.
...
```powershell
powershell -File installers/windows/build-agent.ps1 -ApiUrl "http://server:8000/api" -ApiKey "YOUR_KEY"
```
