...
- **Docs:** Agent deployment → [DEPLOY.md](DEPLOY.md). Central server → [SERVER.md](SERVER.md). Playbook → `playbook/PLAYBOOK-v0.4.1-*.md`. Progress → [PROGRESS.md](PROGRESS.md). Lab runs, results, scripts, and links to playbook/INIT-43/calibration → [docs/lab-runs-and-results.md](docs/lab-runs-and-results.md). Security hardening → [docs/hardening-checklist.md](docs/hardening-checklist.md). CI security jobs and required checks → [docs/ci-security.md](docs/ci-security.md). Security findings and remediation → [docs/security-findings.md](docs/security-findings.md).
...
- **Vocabulary:** Session report terms (timeline types, behavior chains, risk signals, verdict/action) use canonical machine values everywhere; display wording is layered in CLI/dashboard only. See [docs/session-report-vocabulary.md](docs/session-report-vocabulary.md).
