...
| `deploy/` | Agent auto-start templates (Linux systemd, Windows Task). |
| `installers/` | Windows agent and server installer build scripts (PyInstaller, Inno Setup). |
...
```bash
# Bootstrap (run once; installs project with dev extras for tests)
make bootstrap-dev
...
Default dashboard login: `admin@example.com` / `change-me` (unless overridden by `SEED_ADMIN_EMAIL` / `SEED_ADMIN_PASSWORD`). To run tests: after `make bootstrap-dev`, use `make test-collector`, `make test-protocol`, or `make test-api` (see [docs/ci-security.md](docs/ci-security.md)).
...
- **Docs:** Agent deployment → [DEPLOY.md](DEPLOY.md). Central server → [SERVER.md](SERVER.md). Playbook → `playbook/PLAYBOOK-v0.4.1-*.md`. Progress → [PROGRESS.md](PROGRESS.md). Lab runs, results, scripts, and links to playbook/INIT-43/calibration → [docs/lab-runs-and-results.md](docs/lab-runs-and-results.md). Security hardening → [docs/hardening-checklist.md](docs/hardening-checklist.md). CI security jobs and required checks → [docs/ci-security.md](docs/ci-security.md). Security findings and remediation → tracked in [docs/hardening-checklist.md](docs/hardening-checklist.md) and [docs/SECURITY-TECHNICAL-REPORT.md](docs/SECURITY-TECHNICAL-REPORT.md).
...
- **Tests:** Run `make bootstrap-dev` once, then `make test-collector`, `make test-protocol`, or `make test-api` (see [docs/ci-security.md](docs/ci-security.md)). Collector, API, and protocol suites are run separately to avoid package conflicts.
...
- **Vocabulary:** Session report terms (timeline types, behavior chains, risk signals, verdict/action) use canonical machine values everywhere; display wording is layered in CLI/dashboard only.
- **Brand:** Official domain is detecadg.com. Do not use detec.io in official documentation or legal/brand content.
...
3. When the task touches deploy or installer builds, check DEPLOY.md or SERVER.md for the canonical steps.
