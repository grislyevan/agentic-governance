...
- **Sessions:** Agent session list (/sessions) and session detail (timeline, risk summary, behavior chains, evidence, policy simulation); evidence drill-down (timeline row to process/PID/signals); export Markdown/JSON; canned demo session when list empty
