...
### Fixture schema (optional fields for accuracy metrics)

Each fixture JSON may include optional fields used by the calibration metrics harness (`test_calibration_metrics.py`):

- **`telemetry_source`** (optional): `"polling"` | `"native"` | `"mixed"`. Aligns with probe telemetry source; omit for existing fixtures.
- **`label`** (optional): `"tp"` | `"fp"` | `"tn"` | `"fn"` for precision/recall/FPR at policy-relevant thresholds. Positive lab runs = `"tp"`; known benign/negative fixtures = `"tn"` (or `"fp"` if the detector wrongly elevates). Fixtures without `label` are skipped for precision/recall/ECE.
...
| `docs/calibration-metrics.md` | Metric definitions (band accuracy, precision/recall, FPR, ECE) and initial targets |
...
| `collector/tests/test_calibration_metrics.py` | Accuracy metrics: precision, recall, FPR, ECE over labeled fixtures |
