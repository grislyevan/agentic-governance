...
- **Agent (tenant agent key) vs user (JWT/user API key):** Gateway authentication accepts only tenant agent keys (exact match). User roles (owner, admin, analyst, viewer) still apply to HTTP API auth; agent role (`AGENT_ROLE`) has no user_id. References: [api/core/tenant.py](../api/core/tenant.py), [api/gateway.py](../api/gateway.py) `_verify_api_key`.
...
- **Gateway port (8001):** TCP/TLS; auth via first message (tenant agent key + hostname). Connection and per-IP limits.
