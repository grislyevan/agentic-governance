...
**Historical:** The macOS `.pkg` / `installers/macos/` tree was removed. This doc
describes the old installer for archive context only.

This document described macOS installer behavior and failure modes on a clean Mac,
based on the codebase and the agent_reliability_admin_control plan.

## Current Install Flow

- **Artifact:** `.pkg` built by `installers/macos/build-pkg.sh` installs `Detec Agent.app` to `/Applications`.
- **Scripts:** `installers/macos/scripts/preinstall` and `postinstall` run as **root** during install.
...
- Postinstall: `installers/macos/scripts/postinstall`
- Preinstall: `installers/macos/scripts/preinstall`
- Build: `installers/macos/build-pkg.sh`, `installers/macos/distribution.xml`
- LaunchDaemon plist (source): `installers/macos/com.detec.agent.plist`
