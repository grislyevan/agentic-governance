...
- **Single install step:** The installer uses `detec-agent.exe install-service` to register the service, start it, and set failure recovery in one call (see [installers/windows/README.md](../installers/windows/README.md)).
