...
macOS has no dashboard download; use `pip install -e .` and env config.
...
| `installers/windows/README.md` | Updated configure section with tenant agent key terminology |
...
2. **Check service paths:** `Get-WmiObject Win32_Service | Where-Object { $_.Name -match 'Detec' } | Select-Object Name, PathName` shows where each service binary lives. Services pointing to `installers\windows\dist\` are stale dev artifacts. Stop and remove them with `sc.exe stop DetecAgent && sc.exe delete DetecAgent`.
...
| `SEED_API_KEY` | Pinned admin API key (auto-generated, preserved across reinstalls) |
| `SEED_AGENT_KEY` | Pinned tenant agent key (auto-generated, preserved across reinstalls) |
