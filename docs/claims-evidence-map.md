...
| Aider scanner is live-validated. | [lab-runs/LAB-RUN-008-RESULTS.md](../lab-runs/LAB-RUN-008-RESULTS.md), [docs/lab-runs-and-results.md](lab-runs-and-results.md) | accurate | Note live run location/evidence policy in run index. |
| Cline / Continue / LM Studio / GPT-Pilot are validated equivalently to live runs. | [docs/lab-runs-and-results.md](lab-runs-and-results.md), [lab-runs/LAB-RUN-009-RESULTS.md](../lab-runs/LAB-RUN-009-RESULTS.md), [lab-runs/LAB-RUN-010-RESULTS.md](../lab-runs/LAB-RUN-010-RESULTS.md), [lab-runs/LAB-RUN-011-RESULTS.md](../lab-runs/LAB-RUN-011-RESULTS.md), [lab-runs/LAB-RUN-012-RESULTS.md](../lab-runs/LAB-RUN-012-RESULTS.md) | qualified | These are protocol-expected fixture-backed scenarios unless explicitly live-run. |
...
| Process kill tactic is available on Linux/macOS/Windows. | [docs/product-status.md](product-status.md), [collector/engine/enforcement.py](../collector/engine/enforcement.py) | accurate | Keep OS-specific caveats documented. |
| Network null-route tactic is available on Linux/macOS/Windows. | [docs/product-status.md](product-status.md), [collector/engine/enforcement.py](../collector/engine/enforcement.py) | accurate | Linux UID fallback caveat should stay explicit. |
| Proxy injection tactic is available on Linux/macOS/Windows. | [docs/product-status.md](product-status.md), [collector/engine/enforcement.py](../collector/engine/enforcement.py) | qualified | Works when proxy target/config and process model support env-proxy control. |
| Event payloads are SIEM-ready JSON. | [branding/whitepaper.md](../branding/whitepaper.md), [collector/validator.py](../collector/validator.py), [api/tests/test_schema_telemetry.py](../api/tests/test_schema_telemetry.py) | accurate | Avoid claiming zero mapping effort for all SIEMs; field mapping may still be organizational. |
| Evasion testing includes six validated vectors for Claude Code trailer suppression. | [lab-runs/LAB-RUN-EVASION-001-RESULTS.md](../lab-runs/LAB-RUN-EVASION-001-RESULTS.md), [docs/SECURITY-TECHNICAL-REPORT.md](SECURITY-TECHNICAL-REPORT.md) | accurate | Scope to tested vector family (trailer/attribution suppression). |
