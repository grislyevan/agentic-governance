...
### Uninstall (macOS dev / manual LaunchAgent)

There is no bundled macOS uninstaller. Stop a manual LaunchAgent with
`launchctl unload ~/Library/LaunchAgents/ai.agentic-gov.agent.plist`, remove the plist,
and clear config under `~/Library/Application Support/Detec` if present. A prior
packaged agent used root-installed paths; if you still have that layout, unload
`system/com.detec.agent` and remove `/Library/LaunchDaemons/com.detec.agent.plist`,
`/Applications/Detec Agent.app`, and `/Library/Application Support/Detec` as root.
