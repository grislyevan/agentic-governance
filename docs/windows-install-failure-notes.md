...
- **Installer:** Inno Setup script at `installers/windows/installer/detec-agent-setup.iss`.
- **Install target:** `{autopf}\Detec\Agent` (typically `C:\Program Files\Detec\Agent`).
- **Post-install:** The installer runs `detec-agent.exe install` then `detec-agent.exe start` from the app directory, with working directory set to the app directory (see installers/windows/README.md).
...
- Installer: `installers/windows/installer/detec-agent-setup.iss`.
